...
export function AgendaView({
  ...
  const me = useMe().data;
  const events = useCalendarEvents(date, !!me?.google_calendar_connected).data ?? [];
  ...
  function schedule(task: Task, time: string) {
    ...
    const dur = task.time_estimate_min ?? 30;
    update.mutate({
      task,
      patch: {
        block_start: blockInstant(date, time, TZ),
  ...
  function unschedule(task: Task) {
    update.mutate({ task, patch: { block_start: null, block_end: null } });
  }
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="rounded-card border border-border bg-surface p-3 shadow-soft">
        ...
        <div className="relative" style={{ height: (END_HOUR - START_HOUR) * PX_PER_HOUR }}>
          ...
          {/* Google Calendar events (read-only) — sit under the planned time-blocks. */}
          {events
            ...
            .map((e) => {
              ...
              return (
                <div
                  key={e.id}
                  className="absolute right-1 left-12 z-0 overflow-hidden rounded-lg px-2 py-1 text-xs"
                  style={{
                    top,
                    height,
          ...
          {scheduled.map((t) => {
            const startMin = minutesFromMidnight(t.block_start!, TZ);
            const endMin = t.block_end ? minutesFromMidnight(t.block_end!, TZ) : startMin + 30;
            ...
            return (
              <div
                key={t.id}
                className={cn(
                  "absolute right-1 left-12 z-10 overflow-hidden rounded-lg px-2 py-1 text-xs",
                  done && "opacity-60",
                )}
                style={{
                  top,
              ...
              >
                ...
                <span className="text-[10px] text-muted">
                  {timeInTimeZone(t.block_start!, TZ)}
                  {t.block_end ? `–${timeInTimeZone(t.block_end, TZ)}` : ""}
                </span>
              </div>
            );
          })}
