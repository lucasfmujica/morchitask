...
```tsx
<motion.a className="relative px-4 py-2 overflow-hidden" whileHover="hover">
  <motion.span
...
```tsx
<motion.div layout className="bg-white rounded-lg p-4" onClick={() => setExpanded(!expanded)}>
...
export function AnimatedButton({
  ...
  const baseClasses =
    variant === "primary" ? "bg-black text-white" : "bg-white text-black border border-black/10";
...
4. **Use `useReducedMotion` for accessibility:**
   ...
   function Component() {
     ...
     return <motion.div animate={{ y: shouldReduceMotion ? 0 : 20 }} />;
   }
   ```
